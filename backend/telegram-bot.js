...
❌ Неактивних: ${stats.inactive}`);
  ...
  async sendStartMessage(ctx) {
    try {
      const welcomeText = `🗺️ Вітаємо в боті інтерактивної карти "Гуляй, Київ"!

🎯 Тут ти можеш:
• Знайти цікаві локації в Києві
• Дізнатися детальну інформацію про заклади
• Відкрити карту з усіма місцями

📍 Натисни кнопку нижче, щоб відкрити інтерактивну карту!`;
      ...
      setTimeout(async () => {
        try {
          const subscribeText = `📢 Щоб не пропустити нові цікаві локації та оновлення карти, рекомендуємо підписатися на наш канал "Гуляй, Київ"!

✨ В каналі ти знайдеш:
• Нові заклади та локації
• Акції та спеціальні пропозиції
• Цікаві події в місті
